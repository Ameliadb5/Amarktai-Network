 ...
 * POST /api/admin/app-safety — update safety config (adult mode toggle, safe mode).
 *
 * Adult mode enforcement rules:
 *  1. Adult mode can ONLY be enabled when safe mode is explicitly disabled.
 *  2. CSAM / violence / self-harm are ALWAYS blocked regardless of adult mode.
...
export async function POST(request: NextRequest) {
  ...
  let body: { appSlug?: string; safeMode?: boolean; adultMode?: boolean }
  try {
  ...
  const { appSlug, safeMode, adultMode } = body
